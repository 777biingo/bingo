...
client.on(Events.InteractionCreate, async interaction => {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'embed-stworz') {
        const embed = new EmbedBuilder().setTitle('Kreator').setDescription('Użyj przycisków poniżej').setColor(0x4974FF);
    ...
    if (interaction.isButton()) {
        if (interaction.customId === 'btn_download') return interaction.reply({ content: `\`\`\`json\n${JSON.stringify(embedDrafts.get(uid) || {}, null, 2)}\n\`\`\``, ephemeral: true });
        if (interaction.customId === 'btn_save') return interaction.reply({ content: 'Embed zapisany!', ephemeral: true });
